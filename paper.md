...
There are several existing tools that provide similar functionality, such as `raster2dggs`, `Vgrid`, `xESMF`, `pyresample`, and `rasterio`. While these packages are widely used and well maintained, they are built around different design assumptions and do not fully address the requirements of hydrologic and land surface modeling workflows that rely on general unstructured meshes. For example:
...
* `Vgrid` provides a framework for working with popular geodesic and graticule-based DGGS. It supports a range of DGGS-focused conversion workflows, including vector to DGGS, raster to DGGS, and DGGS binning, among others [@Vgrid]. However, the package is limited to DGGS representations and, similar to `raster2dggs`, offers limited geostatistical aggregation options when producing mesh-based outputs, which limits its applicability for modelling workflows that require mass-conserving or cell-level statistical representations of raster data.
...
`uraster` was born out of need to support the conversion of structured topographic datasets into unstructured MPAS meshes, which is widely in Earth system modeling and weather forecasting. The package was further extended to support the Hierarchical Equal Area isoLatitude Pixelisation (HEALpix) mesh, which is also commonly used in the DGGS and astronomy communities, to generate kilo-meter scale land surface datasets for the E3SM project.
...
The generative AI tool, claude code, was used in the development of this software to assist the architecture design, code debugging, and documentation writing. All the code and documentation were reviewed and edited by the authors to ensure accuracy and clarity. 
